...
exports.searchTalent = async (req, res, next) => {
  try {
    const {
      skill_type, location, ratingMin,
      budgetMin, budgetMax, availability,
      page = 1, limit = 12, sort = '-rating.average',
    } = req.query;

    const filter = { isActive: true };

    if (skill_type) filter.skill_type = { $in: Array.isArray(skill_type) ? skill_type : [skill_type] };
    if (location) filter.$or = [
      { 'location.city': new RegExp(location, 'i') },
      { 'location.state': new RegExp(location, 'i') },
      { 'location.country': new RegExp(location, 'i') },
    ];
    if (ratingMin) filter['rating.average'] = { $gte: parseFloat(ratingMin) };
    ...
    if (availability) {
      ...
      filter.availability = {
        $elemMatch: {
          date: {
            $gte: new Date(availDate.setHours(0, 0, 0, 0)),
            $lte: new Date(availDate.setHours(23, 59, 59, 999)),
          },
          isBooked: false,
        },
      };
    ...
    const [talents, total] = await Promise.all([
      TalentProfile.find(filter)
        .populate('user_id', 'name profilepic location')
        .sort(sort)
        .skip(skip)
...
exports.getTalentById = async (req, res, next) => {
  try {
    const talent = await TalentProfile.findOne({ _id: req.params.id, isActive: true })
      .populate('user_id', 'name email profilepic location phone createdAt');
...
exports.createOrUpdateProfile = async (req, res, next) => {
  try {
    ...
    // Parse fields (sent as JSON strings inside FormData)
    ...
    // If images were uploaded, save their paths
    ...
    if (existing) {
      ...
      await profile.populate('user_id', 'name email profilepic');
    }
...
exports.updateAvailability = async (req, res, next) => {
  try {
    ...
    const profile = await TalentProfile.findOne({ _id: req.params.id, user_id: req.user.id });
    if (!profile) return res.status(404).json({ success: false, message: 'Profile not found or unauthorized' });
    profile.availability = availability;
    await profile.save();
    res.json({ success: true, data: profile.availability, message: 'Availability updated' });
  } catch (error) { next(error); }
};
...
exports.getMyProfile = async (req, res, next) => {
  try {
    const profile = await TalentProfile.findOne({ user_id: req.user.id })
      .populate('user_id', 'name email profilepic phone location');
    if (!profile) return res.status(404).json({ success: false, message: 'No talent profile found. Please create one.' });
...
exports.addPortfolioItem = async (req, res, next) => {
  try {
    ...
    if (!profile) return res.status(404).json({ success: false, message: 'Talent profile not found' });
