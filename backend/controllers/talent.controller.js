...
exports.searchTalent = async (req, res, next) => {
  try {
    ...
    if (skill_type) {
      filter.skill_type = { $in: Array.isArray(skill_type) ? skill_type : [skill_type] };
    }
...
exports.getTalentById = async (req, res, next) => {
  try {
    const talent = await TalentProfile.findOne({ _id: req.params.id, isActive: true })
      .populate('user_id', 'name email profile_pic location phone created_at');
...
exports.createOrUpdateProfile = async (req, res, next) => {
  try {
    ...
    const {
      skill_type, bio, experience, portfolio, contact_info,
      hourlyRate, currency, location, languages, tags,
    } = req.body;
...
// ─── Update Availability ──────────────────────────────────────
// PATCH /api/talent/:id/availability
exports.updateAvailability = async (req, res, next) => {
  try {
    const { availability } = req.body; // Array of slot objects

    const profile = await TalentProfile.findOne({ _id: req.params.id, user_id: req.user._id });
    if (!profile) {
...
exports.getMyProfile = async (req, res, next) => {
  try {
    ...
    if (!profile) {
      return res.status(404).json({ success: false, message: 'No talent profile found. Please create one.' });
    }
